...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public void checkUserDataScope(Long userId)
    {
        ...
        if (!SysUser.isAdmin(sysuser.getRoles()))
        {
            SysUser user = new SysUser();
            user.setUserId(userId);
            List<SysUser> users = SpringUtils.getAopProxy(this).selectUserList(null, user);
    ...
    @Override
    public int updateOnlyUser(SysUser user) {
        return userMapper.updateUser(user);
    }
    ...
    @Override
    public Map<Long, SysUser> selectOnlyUserInfoById(List<Long> userId) {
        Map<Long, SysUser> userMap = new HashMap<>();
        List<SysUser> list = sysUserMapper.selectOnlyUserInfoByIds(userId);
        if (list != null && !list.isEmpty()){
            userMap = list.stream().collect(Collectors.toMap(SysUser::getUserId, a -> a, (key1,key2) -> key2));
        }
        return userMap;
    }
}
