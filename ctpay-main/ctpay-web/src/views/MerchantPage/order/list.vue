...
<template>
  <div class="app-container">
    ...
    <km-table :table="tableConfig" @tableChangeCurrent="tableChangeCurrent" @tableChangeSize="tableChangeSize" :toHeight="90">
      <template v-slot:table-after>
        <div class="flex-center">
          <div class="flex-center table-overview" style="width: auto; max-width: 85%">
    ...
    <detailOrder ref="detailOrder"></detailOrder>
     ...
     <!--安全码-->
     <SafeCode ref="safeCodeRef" @submit="handleRepairOrder"></SafeCode>
  </div>
</template>

<script>
import {getInOrderList, getInOrderTotal, repairInOrder, orderUnPaid} from '@/api/order'
...
export default {
  ...
  mounted() {
    ...
    // 更新列表
    EventBus.$on('updateOrderList', () => { this.doSearch()})
  },
  methods: {
    doSearch() {
    ...
    getChannelList() {
      getChannelListAll({}).then(res => {
        const list = res.data || {}
        this.$set(this.searchData[6], 'option', list.map(item => {
          const data = {
    ...
    handleRepair(row) {
      ...
      this.$refs.safeCodeRef.show(row)
    },
