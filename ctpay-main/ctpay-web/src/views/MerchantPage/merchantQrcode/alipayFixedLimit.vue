...
<template>
  <div class="app-container">
    ...
    <alipay-scan-update ref="alipayScanUpdate" @submit="submitAdd"></alipay-scan-update>
  </div>
</template>

<script>
import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
import alipayScanUpdate from './components/alipaycodeUpdate'
import ImagePreview from '@/components/ImagePreview'

export default {
  name: 'alipayFixedLimit',
  components: {
    alipayScanUpdate,
    ImagePreview
  },
  ...
  methods: {
    ...
    handleAdd() {
      this.$refs.alipayScanUpdate.openDialog(true, {
        channelId: '1117'
      })
    },
    // 编辑
    handleUpdate(row) {
      this.$refs.alipayScanUpdate.openDialog(false, row)
    },
    submitAdd(isAdd) {
      if (isAdd) {
        this.doSearch()
