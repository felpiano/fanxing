...
<template>
  <div class="app-container">
    ...
    <km-table
     ...
     <cny-number-update ref="cnyNumberUpdateRef" @submit="submitAdd"></cny-number-update>
   </div>
 </template>
 
 <script>
 import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
 import cnyNumberUpdate from './components/CnyNumberUpdate'
 ...
 export default {
   ...
   methods: {
       ...
       handleAdd() {
         this.$refs.cnyNumberUpdateRef.openDialog(true, {
            channelId: '1107'
         })
       },
       // 编辑
       handleUpdate(row) {
          this.$refs.cnyNumberUpdateRef.openDialog(false, row)
       },
       submitAdd(isAdd) {
         if (isAdd) {
            this.doSearch()
