...
<template>
 <div class="app-container">
   ...
   <km-table
    ...
    <goldBarUpdate ref="goldBarUpdateRef" @submit="submitAdd"></goldBarUpdate>
  </div>
</template>

<script>
import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
import goldBarUpdate from './components/goldBarUpdate.vue'
...
export default {
  ...
  methods: {
      ...
      handleAdd() {
         this.$refs.goldBarUpdateRef.openDialog(true, {
            channelId: '1119'
         })
      },
      // 编辑
      handleUpdate(row) {
         this.$refs.goldBarUpdateRef.openDialog(false, row)
      },
      submitAdd(isAdd) {
         if (isAdd) {
            this.doSearch()
