...
<template>
  <div class="app-container">
    ...
    <km-table
     ...
     <aggregateUpdate ref="aggregateUpdateRef" @submit="submitAdd"></aggregateUpdate>
   </div>
 </template>
 
 <script>
 import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
 import aggregateUpdate from './components/aggregateUpdate'
 ...
 export default {
   ...
   methods: {
       ...
       handleAdd() {
         this.$refs.aggregateUpdateRef.openDialog(true, {
            channelId: '1108'
         })
       },
       // 编辑
       handleUpdate(row) {
          this.$refs.aggregateUpdateRef.openDialog(false, row)
       },
       submitAdd(isAdd) {
         if (isAdd) {
            this.doSearch()
