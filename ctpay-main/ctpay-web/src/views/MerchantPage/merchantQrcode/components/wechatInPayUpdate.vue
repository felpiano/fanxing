  ...
  <el-dialog :title="isAdd ? '添加' : '编辑'" append-to-body :visible.sync="isVisible" width="36%" @close="closeDialog" :destroy-on-close="true">
    <el-form ref="form" :model="form" :rules="rule" label-width="120px" class="dialog-form-height">
     ...
     <el-form-item label="收款账号" prop="accountNumber">
        <el-input v-model.trim="form.accountNumber" placeholder="请输入微信收款账号"></el-input>
      </el-form-item>
      ...
      <el-form-item label="支付链接" prop="accountRemark">
        <el-input v-model.trim="form.accountRemark" placeholder="请复制支付链接到输入框中"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    var validatorAccount = (rule, value, callback) => {
      ...
      // 微信账号， 首尾不能为特殊字符
      if (!regex.test(value)) {
        callback(new Error('收款账号格式不正确，请检查'))
    ...
    return {
      ...
      rule: {
        ...
        // ],
        accountRemark: [
          { required: true, message: '请输入支付链接', trigger: 'change' }
        ],
        qrcodeType: [
