...
<template>
  <el-dialog :title="isAdd ? '添加' : '编辑'" append-to-body :visible.sync="isVisible" width="36%" @close="closeDialog" :destroy-on-close="true">
    <el-form ref="form" :model="form" :rules="rule" label-width="120px" class="dialog-form-height">
      <el-form-item label="收款人姓名" prop="nickName">
        <el-input v-model.trim="form.nickName" placeholder="请输入收款人姓名"></el-input>
      </el-form-item>
      <el-form-item label="钱包编号" prop="accountNumber">
        <el-input v-model.trim="form.accountNumber" placeholder="请输入钱包编号" :disabled="!isAdd"></el-input>
      </el-form-item>
      ...
      <el-form-item label="钱包备注" prop="accountRemark">
        <el-input v-model.trim="form.accountRemark" placeholder="请输入钱包备注"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    var validatorAccount = (rule, value, callback) => {
      ...
      // 钱包编号
      if (!regex.test(value)) {
        callback(new Error('钱包编号格式不正确，请检查'))
