...
<template>
  <el-dialog :title="isAdd ? '添加' : '编辑'" append-to-body :visible.sync="isVisible" width="36%" @close="closeDialog" :destroy-on-close="true">
    <el-form ref="form" :model="form" :rules="rule" label-width="120px" class="dialog-form-height">
      <el-form-item label="银行卡账号" prop="accountNumber">
        <el-input v-model="form.accountNumber" placeholder="请输入银行卡账号" :disabled="!isAdd"></el-input>
      </el-form-item>
      <el-form-item label="银行卡姓名" prop="nickName">
        <el-input v-model.trim="form.nickName" placeholder="请输入银行卡姓名"></el-input>
      </el-form-item>
      <el-form-item label="所属银行" prop="uid">
        <el-input v-model="form.uid" placeholder="请输入所属银行"></el-input>
      </el-form-item>
      <el-form-item label="银行支行" prop="accountRemark">
        <el-input v-model="form.accountRemark" placeholder="请输入银行支行"></el-input>
      </el-form-item>
      ...
      <el-form-item label="二维码" prop="qrcodeUrl" v-if="form.qrcodeType == 1">
...
export default {
  ...
  data() {
    var validatorAccount = (rule, value, callback) => {
      const regex = /^[^\u4e00-\u9fa5!@#$%^&*()_+\[\]{}|;:'",.<>?/~`-].*[^\u4e00-\u9fa5!@#$%^&*()_+\[\]{}|;:'",.<>?/~`-]$/;
      const regex1 = /^[^\s]*$/;
      // 银行卡账号
      if (!regex.test(value)) {
        callback(new Error('银行卡账号格式不正确，请检查'))
