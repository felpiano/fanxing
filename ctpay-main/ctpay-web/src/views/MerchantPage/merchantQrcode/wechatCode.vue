...
<template>
  <div class="app-container">
    ...
    <km-table
     ...
     <WechatUpdate ref="WechatUpdateRef" @submit="submitAdd"></WechatUpdate>
   </div>
 </template>
 
 <script>
 import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
 import WechatUpdate from './components/wechatUpdate'
 ...
 export default {
   ...
   methods: {
       ...
       handleAdd() {
         this.$refs.WechatUpdateRef.openDialog(true, {
            channelId: '1109'
         })
       },
       // 编辑
       handleUpdate(row) {
          this.$refs.WechatUpdateRef.openDialog(false, row)
       },
       submitAdd(isAdd) {
         if (isAdd) {
            this.doSearch()
