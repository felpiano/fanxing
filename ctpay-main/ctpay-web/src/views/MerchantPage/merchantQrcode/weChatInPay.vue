...
<template>
 <div class="app-container">
   ...
   <km-table
    ...
    <wechatInPayUpdate ref="wechatInPayUpdateRef" @submit="submitAdd"></wechatInPayUpdate>
  </div>
</template>

<script>
import {deleteMerchantQrcode, getMerchantQrcodeList, updateMerchantQrcodeStatus} from '@/api/merchantQrcode'
import wechatInPayUpdate from './components/wechatInPayUpdate'
...
export default {
  ...
  methods: {
      ...
      handleAdd() {
         this.$refs.wechatInPayUpdateRef.openDialog(true, {
            channelId: '1122'
         })
      },
      // 编辑
      handleUpdate(row) {
         this.$refs.wechatInPayUpdateRef.openDialog(false, row)
      },
      submitAdd(isAdd) {
         if (isAdd) {
            this.doSearch()
