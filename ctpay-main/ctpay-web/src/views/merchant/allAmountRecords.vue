...
<script>
import { getMerchantaAll, getMerchantAmountRecordsList } from '@/api/merchant'
  ...
  methods: {
    ...
    getMerchantaAllView() {
      getMerchantaAll({
        ...
        let option = res.data || []
        if (this.$store.getters.identity !== 5) {
          this.searchData[5].option = option.map(item => {
            return {
              label: item.userName,
    ...
    exportList() {
      ...
      this.download('merchantAmountRecordsEntity/recordsExport', {
        ...this.tableConfig.searchQuery
      }, `码商资金明细${moment().format('YYYY-MM-DD HHmmss')}.xlsx`)
    },
    // 分页
