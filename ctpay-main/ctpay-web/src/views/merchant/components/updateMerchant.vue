...
<template>
  <el-dialog title="编辑码商" append-to-body :visible.sync="isVisible" width="30%" @close="closeDialog" :destroy-on-close="true">
    <el-form ref="form" :model="form" :rules="rule" label-width="120px" class="dialog-form-height">
      ...
      <el-form-item label="回调白名单" prop="whiteCallbackIpList">
        <div class="flex-center">
          <div v-for="(item, index) in whiteCallbackIpList" :key="index" class="mr10 mb10">
            <el-input style="width: 160px" v-model="item.ip" placeholder="登录IP白名单"></el-input>
            <el-button class="ml10" type="danger" size="small" @click="deleteWhiteIp(index)">删除</el-button>
          </div>
          <el-button class="mb10" type="primary" size="small" @click="addCallbackIp">添加</el-button>
        </div>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    deleteCallbackIp(index) {
      this.whiteCallbackIpList.splice(index, 1)
    },
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          updateMerchantEntity(form).then(res => {
